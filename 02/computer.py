def opcode1(currentposition: int, program, one, two):
   ...
   currentposition += 4
   opcodeswitch(currentposition, program, one, two)

def opcode2(currentposition: int, program, one, two):
...
def opcodeswitch(currentposition: int, program, one, two):
   # identify opcode for current position
   opcode = program[currentposition]
   if opcode == 1:
      opcode1(currentposition, program, one, two)
   elif opcode == 2:
      opcode2(currentposition, program, one, two)
   elif opcode == 99:
      check(program, one, two)
      #print(one, two, program[0])
   else:
      pass
...
def loadprogram(a, b):
   ...
   program[1] = a
   program[2] = b
   opcodeswitch(0, program, a, b)

def check(program, a, b):
   if program[0] == 19690720:
      print("Hit! "+str(a)+" and "+str(b)+" give us "+str((100*a)+b))
   else:
      pass
      #print("Miss ", end="")
